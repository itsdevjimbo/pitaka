...
) : ControllerBase
{
    ...
    private static CategoryType ExpectedCategoryType(RecurringTransactionType type) =>
        type switch
        {
    ...
    private IActionResult? RejectRecurringTransactionCategory(TransactionCategoryVerdict verdict) =>
        verdict switch
        {
    ...
    public async Task<IActionResult> Update(int id, UpdateRecurringTransactionRequest request)
    {
        ...
        {
            return Problem(
                detail: "A recurringTransaction with this name already exists.",
                statusCode: StatusCodes.Status409Conflict
            );
        }
