...
public class GetBudgetCycle(TimeProvider timeProvider)
{
    ...
    private static DateOnly CalendarStart(DateOnly anchor, BudgetPeriod period) =>
        period switch
        {
            ...
            BudgetPeriod.Quarterly => new DateOnly(
                anchor.Year,
    ...
    private static DateOnly CalendarEnd(DateOnly anchor, BudgetPeriod period) =>
        period switch
        {
            BudgetPeriod.Daily => anchor,
            BudgetPeriod.Weekly => CalendarStart(anchor, period).AddDays(6),
            BudgetPeriod.Monthly => new DateOnly(
                anchor.Year,
                anchor.Month,
