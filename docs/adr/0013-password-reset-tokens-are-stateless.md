...
- **A Data Protection key ring persisted to the database becomes a dependency.** `AddDataProtection().PersistKeysToDbContext<PitakaDbContext>()`, via the official `Microsoft.AspNetCore.DataProtection.EntityFrameworkCore` package pinned to its `9.0.x` line (it depends on `Microsoft.EntityFrameworkCore >= 9.0.19` — same major version as the rest of the EF Core stack, so referencing it cost a patch bump, not the EF Core 10 migration it looked like at first glance). The default key ring lives at `~/.aspnet/DataProtection-Keys`, which is per-machine and, in a container, wiped on every deploy — taking every outstanding confirmation and reset token with it. Persisting the ring to the database is what makes tokens survive a redeploy. It adds one table on slice S1's migration. This is the `APP_KEY` role from the Laravel world, played by a rotating key set rather than a single static secret.
