...
`Type` is not a setting on a Category. It is what the Category *is* — an Income "Salary" flipped to Expense is a different category wearing an old name and an old history. Two rules follow from the same premise, and are filed separately: a Transaction's type must agree with its Category's (#76), and a child Category's type must match its parent's (#77).

Enforcement is structural rather than guarded. `CategoryRequest` and `CategoryInput` split into `Create`/`Update` pairs — the same shape `Transaction` and `Account` already use — and the update half carries no `Type`, so the assignment in `UpdateAsync` is deleted rather than protected by a check. `CategorySeeder` and `CreateUserOwnedAsync` are then the only writers of a `Category.Type` in the codebase. This follows the existing treatment of immutable fields here: `UpdateTransactionRequest` has no `Type` and `UpdateAccountRequest` carries `Name` alone. An immutable field is not rejected, it is not accepted.
...
- **The test suite cannot catch a regression here on its own.** Every `Update_*` test in `CategoriesControllerTest` builds an anonymous object rather than a `CategoryRequest`, so re-adding `Type` to the update contract breaks no compilation and fails no assertion. One deliberate test — `PUT` a stray `"type"` and assert the stored value did not move — is the only thing standing between this rule and a future refactor that quietly undoes it.
