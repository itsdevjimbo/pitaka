...
public class GenerateDueRecurringTransactionsTest : IDisposable
{
    ...
    private readonly List<int> _schedulesToPurge = [];
    ...
    public GenerateDueRecurringTransactionsTest(PitakaWebApplicationFactory factory)
    {
        ...
        _generateDueRecurringTransactions =
            _scope.ServiceProvider.GetRequiredService<GenerateDueRecurringTransactions>();
    }
    ...
    public async Task Generate_OverdueRecurringTransaction_MovesToFutureOccurence()
    {
        ...
        Assert.Single(
            await _context
                .Transactions.Where(t => t.RecurringTransactionId == recurringTransaction.Id)
                .ToListAsync()
        );

        var transaction = await _context.Transactions.FirstAsync(t =>
            t.RecurringTransactionId == recurringTransaction.Id
        );
    ...
    public async Task Generate_OneScheduleHasAnUnmappedType_TheRestOfTheRunStillGenerates()
    {
        ...
        var failing = await RecurringTransactionFactory.CreateAsync(
            ...
            startDate: date.AddDays(-1),
            nextRunDate: date,
            type: (RecurringTransactionType)99,
            name: "Unmapped type"
        );
        ...
        Assert.False(
            await _context.Transactions.AnyAsync(t => t.RecurringTransactionId == failing.Id)
        );
        Assert.True(
            await _context.Transactions.AnyAsync(t => t.RecurringTransactionId == healthy.Id)
        );

        var healthyAfter = await _context
    ...
    public async Task Generate_OneScheduleFailsToSave_ItsTrackedStateDoesNotLeakIntoTheNext()
    {
        ...
        var overflowingAccount = await AccountFactory.CreateAsync(
            _context,
            user.Id,
            initialBalance: DecimalColumnCeiling
        );
        var healthyAccount = await AccountFactory.CreateAsync(
            _context,
            user.Id,
            initialBalance: 500
        );
        ...
        var failing = await RecurringTransactionFactory.CreateAsync(
            _context,
            user.Id,
            overflowingAccount.Id,
            startDate: date.AddDays(-1),
            nextRunDate: date,
            amount: DecimalColumnCeiling,
            name: "Overflows"
        );
        _schedulesToPurge.Add(failing.Id);
        ...
        var healthyAfter = await _context
            ...
            .FirstAsync(a => a.Id == healthyAccount.Id);
        Assert.Equal(800, healthyAfter.CurrentBalance);

        var overflowingAfter = await _context
            .Accounts.AsNoTracking()
    ...
    public void Dispose()
    {
        ...
        {
            _context
                .RecurringTransactions.Where(rt => _schedulesToPurge.Contains(rt.Id))
                .ExecuteDelete();
        }
