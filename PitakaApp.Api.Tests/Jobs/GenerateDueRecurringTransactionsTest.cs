...
public class GenerateDueRecurringTransactionsTest : IDisposable
{
    ...
    [Fact]
    public async Task Generate_WithExistingRetiredCategory_GeneratesTransactionWithCategory()
    {
        var user = await UserFactory.CreateAsync(_context);
        var account = await AccountFactory.CreateAsync(_context, user.Id);
        ...
        var transaction = await _context.Transactions.SingleAsync(t =>
            t.RecurringTransactionId == recurringTransaction.Id
        );
        ...
        Assert.Equal(
            $"Generated by recurring transaction: {recurringTransaction.Name}",
